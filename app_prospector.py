...
st.set_page_config(page_title="IA Prospector Live", layout="wide", page_icon="🕵️‍♂️")
...
# ---Titre ---
col1title, col2title, col3title = st.columns(3)
with col2title:
    st.title("🕵️‍♂️ IA Prospector Live")
    st.caption("Intelligence Insee & Web en temps réel")

st.markdown("---")

# --- BARRE DE RECHERCHE ---
col1p, col2p, col3p = st.columns([1, 4, 1])
with col2p:
    user_prompt = st.text_input("Que recherchez-vous ?", placeholder="Ex: Les 5 plus grosses industries à Dunkerque avec téléphone...")
    c1b, c2b, c3b = st.columns([3, 1, 4])
    with c2b:
...
def run_ia_logic(prompt):
    with st.status("🧠 Réflexion de l'IA en cours...", expanded=True) as status:
        ...
        # Commande Gemini avec consigne explicite contre la délégation
        cmd = ["gemini", "--approval-mode", "yolo", f"SYSTEM: N'utilise JAMAIS l'agent 'generalist'. Fais tout toi-même étape par étape. REQUÊTE: {prompt}"]
...
def display_results():
    ...
    if list_of_files:
        ...
        st.markdown("---")
        col1sub, col2sub, col3sub = st.columns([2.5, 4, 1])
        with col2sub:
        ...
        # 2. BOUTON DOWNLOAD
        col1dl, col2dl, col3dl = st.columns([2.5, 1, 3])
        with col2dl:
        ...
        # 4. VISUALISATIONS
        colsubb1, colsubb2, colsubb3 = st.columns([2, 4, 1])
        with colsubb2:
        ...
        st.info("💡 Saisissez une recherche pour générer des données.")
